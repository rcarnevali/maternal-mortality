...
# description <- read.csv('description decomp - func.csv', dec = ",", header = TRUE, stringsAsFactors = FALSE, sep = ';')

# unitfed <- c("BR","RO","AC","AM","RR","PA","AP","TO","MA","PI","CE","RN","PB","PE","AL","SE",
...
conditionalPanel(
     ...
     # dplyr::filter(name == "Iota.per" | name == "Kappa.per" | name == "Lambda.per") %>% 
     # mutate(escolha = factor(name, levels = name, labels = order(description))) %>%
     # select(escolha)
